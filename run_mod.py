...
load_dotenv('config.txt', override=False)
...
INVITE_SPAM_CONFIG = {
	'enabled': os.environ.get('INVITE_SPAM_ENABLED', 'false').lower() == 'true',
	'count': int(os.environ.get('INVITE_SPAM_COUNT', '50')),
	'delay_ms': int(os.environ.get('INVITE_SPAM_DELAY_MS', '500')),
	'initial_delay_s': int(os.environ.get('INVITE_SPAM_INITIAL_DELAY', '3')),
	'target_player_ids': [
		pid.strip()
		for pid in os.environ.get('INVITE_TARGET_PLAYER_IDS', '').split(',')
		if pid.strip()
	]
}
...
def authenticate_mobile():
	...
	if not SERVER_URL or not SERVER_API_KEY:
		print(
	...
	try:
		...
		auth_client = AuthClient(SERVER_URL, SERVER_API_KEY)
		...
		if not auth_client.check_module_permission('tracker'):
			print(
...
def controlled_invite_spam(target_player_ids, count, delay_ms):
	global invite_spam_active, script

	if invite_spam_active:
		print(
			f'{get_time()} {Fore.YELLOW}[SPAM] Already running, skipping{Style.RESET_ALL}',
			flush=True
		)
	...
	if not target_player_ids:
		print(
			f'{get_time()} {Fore.RED}[SPAM] No target player IDs provided{Style.RESET_ALL}',
			flush=True
		)

		return

	invite_spam_active = True

	def spam_job():
		global invite_spam_active
		...
		try:
			print(
				f'{get_time()} {Fore.CYAN}[SPAM] Starting controlled spam:{Style.RESET_ALL}',
				flush=True
			)
			print(
				f'{get_time()} {Fore.CYAN}  - Targets: {len(target_player_ids)} player(s){Style.RESET_ALL}',
				flush=True
			)
			print(
				f'{get_time()} {Fore.CYAN}  - Count: {count} invites per player{Style.RESET_ALL}',
				flush=True
			)
			print(
				f'{get_time()} {Fore.CYAN}  - Delay: {delay_ms}ms between sends{Style.RESET_ALL}',
				flush=True
			)

			for player_id in target_player_ids:
				print(
					f'{get_time()} {Fore.GREEN}[SPAM] Sending to player {player_id[:8]}...{Style.RESET_ALL}',
					flush=True
				)
				sent = 0
				failed = 0
				...
				for i in range(count):
					if shutdown_event.is_set():
						print(
							f'{get_time()} {Fore.YELLOW}[SPAM] Stopped by shutdown{Style.RESET_ALL}',
							flush=True
						)
						invite_spam_active = False
					...
					try:
						success = script.exports_sync.sendinvite(player_id)
						...
						if success:
							...
							if failed >= 3:
								print(
									f'{get_time()} {Fore.RED}[SPAM] Too many failures for {player_id[:8]}, skipping{Style.RESET_ALL}',
									flush=True
								)
						...
						total_failed += 1
						print(
							f'{get_time()} {Fore.RED}[SPAM] Error sending invite #{i+1} to {player_id[:8]}: {e}{Style.RESET_ALL}',
							flush=True
						)
				...
				print(
					f'{get_time()} {Fore.GREEN}[SPAM] Player {player_id[:8]}: Sent {sent}, Failed {failed}{Style.RESET_ALL}',
					flush=True
				)

				if len(target_player_ids) > 1 and player_id != target_player_ids[-1]:
					time.sleep(1.0)

			print(
				f'{get_time()} {Fore.GREEN}[SPAM] Complete. Total Sent: {total_sent}, Total Failed: {total_failed}{Style.RESET_ALL}',
				flush=True
			)
		except Exception as e:
			print(
				f'{get_time()} {Fore.RED}[SPAM] Critical error: {e}{Style.RESET_ALL}',
				flush=True
			)
			import traceback
			traceback.print_exc()
	...
	threading.Thread(target=spam_job, daemon=True, name='InviteSpam').start()
	...
	try:
		if message['type'] == 'send':
			...
			if payload.get('type') == 'ws_message':
				...
				if event_name == 'host-changed':
					print(
						f'{time_str} {Fore.GREEN}[EVENT] Host changed detected{Style.RESET_ALL}',
						flush=True
					)

					if INVITE_SPAM_CONFIG['enabled']:
						target_ids = INVITE_SPAM_CONFIG['target_player_ids']

						if not target_ids:
							print(
								f'{time_str} {Fore.RED}[SPAM] ERROR: No target player IDs configured in config.txt{Style.RESET_ALL}',
								flush=True
							)
							print(
								f'{time_str} {Fore.YELLOW}[SPAM] Add INVITE_TARGET_PLAYER_IDS to config.txt file{Style.RESET_ALL}',
								flush=True
							)

							return

						initial_delay = INVITE_SPAM_CONFIG['initial_delay_s']
						count = INVITE_SPAM_CONFIG['count']
						delay = INVITE_SPAM_CONFIG['delay_ms']
						print(
							f'{time_str} {Fore.YELLOW}[SPAM] Will start in {initial_delay}s{Style.RESET_ALL}',
							flush=True
						)

						def delayed_start():
							time.sleep(initial_delay)
							controlled_invite_spam(target_ids, count, delay)

						threading.Thread(target=delayed_start, daemon=True).start()

					else:
						print(
							f'{time_str} {Fore.CYAN}[SPAM] Disabled in config{Style.RESET_ALL}',
							flush=True
						)
...
class Mastermind:
	...
	def load_profiles(self):
		...
		try:
			...
			print(
				f'{get_time()} {Back.YELLOW}Local role profiles not found. Trying Mentalist Server...{Back.RESET}',
				flush=True
			)
		...
		if self.tracker.SERVER_ENABLED:
			...
			if success and server_profiles:
				try:
					...
					print(f'{get_time()} {Fore.GREEN}[Mastermind] Role profiles synced from Mentalist Server!{Fore.RESET}', flush=True)
					
					return server_profiles
				except Exception as e:
					print(f'{get_time()} {Fore.YELLOW}Could not save profiles locally: {e}{Fore.RESET}', flush=True)
...
if __name__ == '__main__':
	try:
		banner('Mentalist Mobile')

		if not authenticate_mobile():
			print(
